...
void CustomerService::checkout(User& user, std::vector<OrderItem>& cart) {
    ...
    Order newOrder(orderId , user.getusername() , user.getpasswordHash() , cart , totalCost , destination , address , "Pending" );

    orders.push_back(newOrder);

    std::string packageId = Common::nextPackageId();
    Package pack(packageId , orderId , user.getusername() , user.getpasswordHash() , cart , destination , address , "Pending" , "Pending computation");
...
void CustomerService::displayOrderHistoryFromOrders(const User& user) {
    ...
    for (const auto& order : orders) {
        if (order.getpassworHash() == user.getpasswordHash()){
